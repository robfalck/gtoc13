...
def add_arc(args):
    ...
    success, sol, prob = solve_arc(from_body=body_1,
                                   to_body=body_2,
                                   t1=t1_year,
                                   t2=t1_year + args.flyby_dt,
                                   v_in_1 = v_in_1,
                                   v_out_1 = v_out_1,
                                   opt_v_in_1 = not args.fix_vin1,
    ...
    new_solution_file = seed_file.parent / f'{body_1}_{body_2}.txt'

    sol.write_to_file(new_solution_file, precision=11)
    print(f"Solution written to {new_solution_file}")

    if success:

        with open(seed_file, 'r') as f1:
            seed_data = f1.read()

        with open(new_solution_file, 'r') as f2:
            new_data = f2.read()

        combined_file = seed_file.stem + f'_{args.body}.txt'

        with open(combined_file, 'w') as f3:
            f3.write(seed_data)
            f3.write('\n')
            f3.write(new_data)

        print(f"New Arc Solution written to {new_solution_file}")
        print(f"Unoptimized combined solution to {combined_file}")

    print('Reports:', prob.get_reports_dir())
