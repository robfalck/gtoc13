...
def get_phase(num_nodes, control):
    ...
    phase = dm.Phase(ode_class=ode_cls, transcription=tx)
    ...
    if control == 1:
        phase.add_control(
            "u_n", units="unitless", shape=(3,), opt=True, val=np.ones((3,)), targets=["u_n"]
        )
        phase.add_path_constraint("u_n_norm", equals=1.0)
        phase.add_path_constraint("cos_alpha", lower=0.0)
    elif control == 0:
        phase.add_parameter("u_n", units="unitless", shape=(3,), val=np.zeros((3,)), opt=False)
...
def get_dymos_serial_solver_problem(
    bodies: Sequence[int],
    ...
    for i in range(N):
        ...
        prob.model.connect(
            "event_pos", f"traj.arc_{i}.initial_states:r", src_indices=om.slicer[i, ...]
        )
        prob.model.connect(
            "event_pos", f"traj.arc_{i}.final_states:r", src_indices=om.slicer[i + 1, ...]
        )
    ...
    prob.model.connect("v_in_out_comp.flyby_v_in", "flyby_comp.v_in")

    prob.model.connect("v_in_out_comp.flyby_v_out", "flyby_comp.v_out")
    ...
    prob.model.add_subsystem("score_comp", ScoreComp(bodies=bodies), promotes_outputs=["J"])

    prob.model.connect("event_pos", "score_comp.body_pos", src_indices=om.slicer[1:, ...])
    ...
    if len(planet_flyby_idxs) > 0:
        prob.model.add_constraint(
            "flyby_comp.h_p_defect", indices=planet_flyby_idxs, upper=0.0, ref=1000.0
        )
    ...
    prob.model.traj.phases.arc_0.add_boundary_constraint(
        "v", loc="initial", indices=[1, 2], equals=0.0
    )
    ...
    prob.driver.opt_settings["acceptable_tol"] = (
        1.0  # Large value means we don't care about optimality
    )
    prob.driver.opt_settings["acceptable_constr_viol_tol"] = 1.0e-6  # Must satisfy constraints
    prob.driver.opt_settings["acceptable_dual_inf_tol"] = (
        1.0e10  # Don't care about dual infeasibility
    )
    prob.driver.opt_settings["acceptable_compl_inf_tol"] = (
        1.0e10  # Don't care about complementarity
    )
    # Number of iterations at acceptable level before terminating
    prob.driver.opt_settings["acceptable_iter"] = (
        5  # Accept after 5 consecutive "acceptable" iterations
    )
...
def set_initial_guesses(prob, bodies, flyby_times, t0, controls,
    ...
    if guess_solution is not None:
        non_flyby_arcs = [
            arc for arc in guess_solution.arcs if isinstance(arc, (PropagatedArc, ConicArc))
        ]
...
def create_solution(prob, bodies, controls=None, filename=None):
    ...
    solution = GTOC13Solution(arcs=arcs, comments=[], objective_J=J)
    ...
    solution.plot(show_bodies=True, save_path=plot_file, E_end=E_end, J=J)
...
if __name__ == "__main__":
    ...
    prob = get_dymos_serial_solver_problem(
        bodies=bodies, num_nodes=20, warm_start=False, default_opt_prob=True
    )
    prob.setup()
